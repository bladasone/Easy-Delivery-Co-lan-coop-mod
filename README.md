...
- Авторитет у хоста: хост принимает/пр��меняет ключевые изменения и рассылает снапшоты
...
> ⚠️ **Важно!** В файле `.csproj` пути к зависимостям игры (`HintPath`) настроены для **non-Steam версии** Easy Delivery Co (папка `..\Easy Delivery Co\`). Если у вас установлена **Steam-версия**, название папки игры может отличаться (например, `EasyDeliveryCo steam version`). Если при сборке возникают ошибки о недостающих DLL-ссылках:
> - Вручную обновите пути в `EasyDeliveryCoLanCoop.csproj` в соответствии с расположением вашей папки игры, **либо**
> - Переривязите ссылки на DLL-библиотеки игры в вашей IDE (Visual Studio / Rider).
...
> ⚠️ **Important!** The `.csproj` file has library dependency paths (`HintPath`) configured for the **non-Steam version** of Easy Delivery Co (folder: `..\Easy Delivery Co\`). If you have the **Steam version** installed, your game folder may have a different name (e.g., `EasyDeliveryCo steam version`). If the build fails due to missing DLL references:
